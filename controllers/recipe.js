...
export async function getRecipeDetailsByRecipeId(req, res) {
    ...
    const query = Recipe.aggregate([
        ...
        {
            "$project": { "heroBanner": 1, "title": 1, "description": 1, "servings": 1, "nutrients": 1, "Author._id": 1, "Author.thumbnail": 1, "Author.name": 1, "Author.authorSocials": 1, "Author.isVerified": 1, "viewCount": { "$size": "$Views" }, "commentCount": { "$size": "$Comments"}, "steps": 1, "ingridients": 1, "duration": 1, "budget": 1, "likeCount": {"$size": "$Likes"} }
        },
    ])
